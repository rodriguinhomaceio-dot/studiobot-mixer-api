...
// ─── Cadeia de processamento de VOZ a partir de preset ───────────────
function buildVoiceFilterChain(sampleRate, voicePresetName = "broadcast_ssl", includeAir = true) {
  ...
  if (hasHighRate && includeAir && preset.air) {
    for (const band of preset.air) {
      chain.push(`equalizer=f=${band.freq}:t=q:w=1.4:g=${band.gain}`);
    }
  }
...
async function processJingleMix(opts) {
  ...
  runFfmpeg([
    "-i", voiceFile,
    "-af", buildVoiceFilterChain(sampleRate, voicePresetName, false).join(","),
    "-ar", String(sampleRate),
    "-ac", "1",
    "-y", voiceDspFile,
  ]);
